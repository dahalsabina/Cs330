...
@app.route('/api/v1/view_cart', methods=['GET'])
@cross_origin()
def view_cart():
    if 'cart' not in session:
        session['cart'] = {}  # Initialize empty cart if not present
...
def calculate_total_price(cart):
    ...
    for item in cart:
        ...
        if item_type == 'mens':
            product = MensClothing.query.get(item_id)
        elif item_type == 'womens':
            product = WomensClothing.query.get(item_id)
        else:
            continue
...
def add_to_cart():
    ...
    item_type = data['type']  # 'mens' or 'womens'
    item_id = data['id']
    ...
    session['cart'].append({'type': item_type, 'id': item_id, 'quantity': quantity})
...
@app.route('/api/v1/edit_cart', methods=['POST'])
@cross_origin()
def edit_cart():
    data = request.get_json()
    item_type = data['type']  # 'mens' or 'womens'
